...
@component
class CustomBedrockChatGenerator:
    """
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return default_to_dict(
            self,
            aws_access_key_id=(
                self.aws_access_key_id.to_dict() if self.aws_access_key_id else None
            ),
            aws_secret_access_key=(
                self.aws_secret_access_key.to_dict()
                if self.aws_secret_access_key
                else None
            ),
            aws_session_token=(
                self.aws_session_token.to_dict() if self.aws_session_token else None
            ),
            aws_region_name=(
                self.aws_region_name.to_dict() if self.aws_region_name else None
            ),
            aws_profile_name=(
                self.aws_profile_name.to_dict() if self.aws_profile_name else None
            ),
            model=self.model,
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "CustomBedrockChatGenerator":
        """
    ...
    def run(
        ...
        messages_list = [
            {"role": msg.role.value, "content": [{"text": msg.text}]}
            for msg in messages
        ]
        if document:
            messages_list[len(messages_list) - 1]["content"].append(document)
        ...
        if self.guardrailConfig:
            params["guardrailConfig"] = self.guardrailConfig
    ...
    def extract_replies_from_response(
            ...
            if message["role"] == "assistant":
                ...
                base_meta = {
                    ...
                    "usage": {
                        ...
                        "total_tokens": response_body.get("usage", {}).get(
                            "totalTokens", 0
                        ),
                    },
                }
                    ...
                    if "text" in content_block:
                        replies.append(
                            ChatMessage.from_assistant(
                                content=content_block["text"], meta=base_meta.copy()
                            )
                        )
        ...
        for event in response_stream:
            if "contentBlockStart" in event:
                ...
                if current_tool_use:
                    ...
                    replies.append(
                        ChatMessage.from_assistant(
                            content=tool_content, meta=base_meta.copy()
                        )
                    )
                elif current_content:
                    replies.append(
                        ChatMessage.from_assistant(
                            content=current_content, meta=base_meta.copy()
                        )
                    )
